...
module.exports = async (req, res) => {
  try {
    ...
    const q = req.query || {};
    const input = String(q.product_id || "").trim();
    const debug = String(q.debug || "") === "1";
    let wid = isWID(input) ? input.toUpperCase() : widFromHash(input);
    if (!isWID(wid)) {
      return send(res, 200, {
        ok: false, error_code: "MISSING_WID",
        message: "Passe WID (MLB...) ou link com #...wid=MLB...",
        http_status: 400, _debug: debug ? { input } : undefined
      });
    }
    ...
    if (!r.ok) {
      return send(res, 200, {
        ...
        http_status: r.status, details: { upstream_url: r.url },
        _debug: debug ? { wid } : undefined
      });
    }
    const body = r.data || {};
    ...
    if (!price) {
      return send(res, 200, {
        ok: false, error_code: "NO_PRICE",
        message: "Anúncio sem preço disponível",
        http_status: 404, details: { upstream_url: r.url },
        _debug: debug ? { wid, body } : undefined
      });
    }
    return send(res, 200, {
      ok: true, price, source: "item", product_id: wid, item_id: wid,
      sold_winner: Number.isFinite(body.sold_quantity) ? body.sold_quantity : null,
      sold_catalog_total: null, fetched_at: new Date().toISOString()
    });
  } catch (e) {
    return send(res, 200, { ok: false, error_code: "INTERNAL", http_status: 500, message: String(e?.message || e) });
  }
};
